            ...
            if(data){
                ...
                for(var key in data){
                    ...
                    if(/^.*[РрД][1-9]?$/.test(data[key].contract)){
                        ...
                        tr.className = 'orders';
                        if(ri==4){
                            tr.className += ' tablerow tablenone';
                        }else{
                            tr.className += ' tablerow visible';
                        }
                    }
                    ...
                    td23.innerHTML = ('0' + term.getDate()).slice(-2) + '.' + ('0' + (term.getMonth() + 1)).slice(-2);
                    td02.innerHTML = ('0' + plan.getDate()).slice(-2) + '.' + ('0' + (plan.getMonth() + 1)).slice(-2);
                    td02.addEventListener("click",showChangeDate);
                    td03.innerHTML = data[key].dis;
                    td04.innerHTML = data[key].tech;
                    contentTD(data[key].tech_end,td05,'tech_end');
                    contentTD(data[key].mater,td06,'mater');
                    contentTD(data[key].raspil,td07,'raspil');
                    contentTD(data[key].cpu,td08,'cpu');
                    contentTD(data[key].kromka,td09,'kromka');
                    contentTD(data[key].krivolin,td10,'krivolin');
                    contentTD(data[key].pris_end,td11,'pris_end');
                    contentTD(data[key].gnutje,td12,'gnutje');
                    contentTD(data[key].emal,td13,'emal');
                    contentTD(data[key].pvh,td14,'pvh');
                    contentTD(data[key].shpon,td15,'shpon');
                    contentTD(data[key].photo,td16,'photo');
                    contentTD(data[key].pesok,td17,'pesok');
                    contentTD(data[key].vitrag,td18,'vitrag');
                    contentTD(data[key].oracal,td19,'oracal');
                    contentTD(data[key].steklo,td20,'steklo');
                    contentTD(data[key].fas,td21,'fas');
                    contentTD(data[key].upak_end,td22,'upak_end');
